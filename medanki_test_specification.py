...
class IClassificationService(Protocol):
    ...
    async def classify(self, chunk: Chunk, exam_type: ExamType) -> list[TopicMatch]:
        """Classify a chunk against the specified taxonomy."""
        ...

    async def classify_dual(self, chunk: Chunk) -> dict[ExamType, list[TopicMatch]]:
...
class IGenerationService(Protocol):
    ...
    async def generate_vignette(self, chunk: ClassifiedChunk) -> VignetteCard | None:
...
class IValidationService(Protocol):
    ...
    async def check_duplicate(
        self, card: ClozeCard | VignetteCard, existing_cards: list[ClozeCard | VignetteCard]
    ) -> bool:
...
class IExportService(Protocol):
    ...
    def build_deck(
        self, cards: list[ClozeCard | VignetteCard], exam_type: ExamType, deck_name: str
    ) -> Path:
        """Build an .apkg file from cards."""
        ...

    def build_tags(self, topics: list[TopicMatch], source: str | None) -> list[str]:
...
class IVectorStore(Protocol):
    ...
    async def hybrid_search(
        self, collection: str, query_text: str, query_vector: list[float], alpha: float, limit: int
    ) -> list[dict[str, Any]]:
...
class ITaxonomyService(Protocol):
    ...
    async def get_topic_embeddings(self, exam_type: ExamType) -> dict[str, list[float]]:
...
class TestPDFIngestion:
    ...
    def test_ingest_simple_pdf_returns_document(
        self, pdf_ingestor: IIngestionService, simple_pdf_path: Path
    ):
    ...
    def test_ingest_pdf_extracts_text_content(
        self, pdf_ingestor: IIngestionService, pdf_with_known_content: tuple[Path, str]
    ):
    ...
    def test_ingest_pdf_preserves_section_structure(
        self, pdf_ingestor: IIngestionService, pdf_with_headers: Path
    ):
    ...
    def test_ingest_pdf_extracts_page_numbers(
        self, pdf_ingestor: IIngestionService, multi_page_pdf: Path
    ):
    ...
    def test_scanned_pdf_uses_ocr_extractor(self, pdf_extraction_strategy, scanned_pdf_path: Path):
    ...
    def test_pdf_with_tables_uses_docling(self, pdf_extraction_strategy, table_heavy_pdf: Path):
    ...
    def test_pdf_with_equations_uses_appropriate_extractor(
        self, pdf_extraction_strategy, math_heavy_pdf: Path
    ):
    ...
    def test_tables_extracted_as_structured_data(
        self, pdf_ingestor: IIngestionService, pdf_with_tables: Path
    ):
    ...
    def test_drug_dosage_table_preserves_values(
        self, pdf_ingestor: IIngestionService, pdf_with_drug_table: tuple[Path, dict[str, str]]
    ):
...
class TestAudioIngestion:
    ...
    def test_ingest_audio_returns_document(
        self, audio_ingestor: IIngestionService, sample_audio_path: Path
    ):
    ...
    def test_audio_segments_by_natural_pauses(
        self, audio_ingestor: IIngestionService, lecture_with_pauses: Path
    ):
...
class TestContentTypeDetection:
    ...
    def test_content_type_detection_by_filename(
        self, ingestor: IIngestionService, filename: str, expected_type: ContentType, tmp_path: Path
    ):
...
class TestChunking:
    ...
    def test_chunk_produces_non_empty_chunks(
        self, chunker: IChunkingService, sample_document: Document
    ):
    ...
    def test_chunk_respects_max_token_limit(
        self, chunker: IChunkingService, long_document: Document, max_tokens: int = 512
    ):
    ...
    def test_chunk_respects_min_token_limit(
        self, chunker: IChunkingService, sample_document: Document, min_tokens: int = 100
    ):
    ...
    def test_chunk_ids_are_unique(self, chunker: IChunkingService, sample_document: Document):
    ...
    def test_chunks_track_document_id(self, chunker: IChunkingService, sample_document: Document):
    ...
    def test_chunk_preserves_drug_names(
        self, chunker: IChunkingService, document_with_drug_names: Document
    ):
    ...
    def test_chunk_preserves_anatomical_terms(
        self, chunker: IChunkingService, document_with_anatomy: Document
    ):
    ...
    def test_chunk_preserves_lab_values_with_units(
        self, chunker: IChunkingService, document_with_labs: Document
    ):
    ...
    def test_chunk_preserves_abbreviations(
        self, chunker: IChunkingService, document_with_abbreviations: Document
    ):
    ...
    def test_chunk_respects_section_boundaries(
        self, chunker: IChunkingService, document_with_sections: Document
    ):
    ...
    def test_chunk_detects_topic_shifts(
        self, chunker: IChunkingService, document_with_topic_shift: Document
    ):
        ...
        mixed_chunks = [
            c for c in chunks if "heart" in c.text.lower() and "kidney" in c.text.lower()
        ]
    ...
    def test_chunk_extracts_medical_entities(
        self, chunker: IChunkingService, document_with_entities: Document
    ):
...
class TestEmbedding:
    ...
    def test_embed_returns_correct_dimensions(
        self, embedder: IEmbeddingService, sample_texts: list[str]
    ):
    ...
    def test_embed_single_matches_batch(self, embedder: IEmbeddingService, sample_text: str):
    ...
    def test_similar_texts_have_similar_embeddings(self, embedder: IEmbeddingService):
    ...
    def test_embed_handles_empty_string(self, embedder: IEmbeddingService):
...
class TestClassification:
    ...
    def test_classify_returns_topic_matches(
        self, classifier: IClassificationService, cardiology_chunk: Chunk
    ):
        """Classification returns topic matches."""
        topics = pytest.run_sync(classifier.classify(cardiology_chunk, ExamType.USMLE_STEP1))
    ...
    def test_classify_pharmacology_content(
        self, classifier: IClassificationService, pharmacology_chunk: Chunk
    ):
        """Pharmacology content is classified to drug-related topics."""
        topics = pytest.run_sync(classifier.classify(pharmacology_chunk, ExamType.USMLE_STEP1))

        topic_paths = [t.path_str.lower() for t in topics]
        disciplines = [t.path[1].lower() if len(t.path) > 1 else "" for t in topics]

        assert any("pharmacology" in p or "drug" in p for p in topic_paths) or any(
            "pharmacology" in d for d in disciplines
        )
    ...
    def test_classify_returns_multiple_topics(
        ...
        topics = pytest.run_sync(classifier.classify(multi_topic_chunk, ExamType.USMLE_STEP1))

        # Should have both Cardiology and Pharmacology
        assert len(topics) >= 2

    def test_classify_respects_max_topics(
        self, classifier: IClassificationService, multi_topic_chunk: Chunk, max_topics: int = 5
    ):
    ...
    def test_classify_respects_base_threshold(
        self, classifier: IClassificationService, sample_chunk: Chunk, base_threshold: float = 0.65
    ):
        """All returned topics exceed base confidence threshold."""
        topics = pytest.run_sync(classifier.classify(sample_chunk, ExamType.USMLE_STEP1))
    ...
    def test_classify_dual_returns_both_taxonomies(
        self, classifier: IClassificationService, biochemistry_chunk: Chunk
    ):
    ...
    def test_mcat_classification_uses_foundational_concepts(
        ...
        topics = pytest.run_sync(classifier.classify(biochemistry_chunk, ExamType.MCAT))
    ...
    def test_usmle_classification_uses_organ_systems(
        ...
        has_system = any(any(sys in t.path_str.lower() for sys in organ_systems) for t in topics)
    ...
    def test_classify_handles_abbreviations(self, classifier: IClassificationService):
        ...
        topics = pytest.run_sync(classifier.classify(chunk, ExamType.USMLE_STEP1))

        # Should recognize CHF = Congestive Heart Failure = Cardiovascular
        topic_paths = [t.path_str.lower() for t in topics]
        assert any("cardio" in p or "heart" in p for p in topic_paths)

    def test_classify_handles_rare_abbreviations(self, classifier: IClassificationService):
...
class TestVectorStore:
    ...
    def test_hybrid_search_balances_bm25_and_vector(self, vector_store: IVectorStore):
        ...
        pytest.run_sync(vector_store.upsert("test", vectors, metadata, ids))
...
class TestClozeGeneration:
    ...
    def test_generate_cloze_returns_cards(
        self, generator: IGenerationService, classified_chunk: ClassifiedChunk
    ):
    ...
    def test_cloze_contains_deletion_syntax(
        ...
        for card in cards:
            assert cloze_pattern.search(card.text), f"Card missing cloze syntax: {card.text}"

    def test_cloze_deletion_has_short_answer(
        self, generator: IGenerationService, classified_chunk: ClassifiedChunk, max_words: int = 4
    ):
    ...
    def test_cloze_tests_single_fact(
        ...
        for card in cards:
            ...
            assert 1 <= len(deletions) <= 3, f"Card has {len(deletions)} deletions, should be 1-3"
    ...
    def test_cloze_cards_are_unique(
        ...
        assert unique_concepts >= len(cards) * 0.8, "Too many duplicate concepts in generated cards"
    ...
    def test_cloze_is_self_contained(
        ...
        for card in cards:
            ...
            assert len(card.text) >= 30, f"Card too short to be self-contained: {card.text}"
    ...
    def test_pharmacology_cloze_pattern(
        self, generator: IGenerationService, pharmacology_classified_chunk: ClassifiedChunk
    ):
        """Pharmacology cards follow drug-mechanism-indication pattern."""
        cards = pytest.run_sync(generator.generate_cloze(pharmacology_classified_chunk, count=2))
        ...
        has_pharma_structure = any(
            any(term in card.text.lower() for term in pharma_terms) for card in cards
        )
    ...
    def test_anatomy_cloze_includes_relationships(
        self, generator: IGenerationService, anatomy_classified_chunk: ClassifiedChunk
    ):
        """Anatomy cards include spatial/functional relationships."""
        cards = pytest.run_sync(generator.generate_cloze(anatomy_classified_chunk, count=2))
        ...
        has_relationships = any(
            any(term in card.text.lower() for term in relationship_terms) for card in cards
        )
...
class TestVignetteGeneration:
    ...
    def test_generate_vignette_returns_card(
        self, generator: IGenerationService, clinical_classified_chunk: ClassifiedChunk
    ):
    ...
    def test_vignette_ends_with_question(
        ...
        if card:
            assert card.front.strip().endswith("?"), "Vignette should end with a question"
    ...
    def test_vignette_has_concise_answer(
        ...
        if card:
            ...
            assert word_count <= max_words, f"Answer too long ({word_count} words): {card.answer}"
    ...
    def test_vignette_has_explanation(
        ...
        if card:
            assert len(card.explanation) >= 50, "Explanation too short"
    ...
    def test_vignette_question_types(
        ...
        if card:
            ...
            has_valid_question = any(ending in front_lower for ending in valid_endings)
...
class TestCardValidation:
    ...
    def test_validate_valid_cloze_passes(self, validator: IValidationService):
    ...
    def test_validate_missing_cloze_fails(self, validator: IValidationService):
    ...
    def test_validate_long_cloze_answer_fails(self, validator: IValidationService):
        ...
        assert not result.is_valid
        assert (
            "too long" in " ".join(result.issues).lower()
            or result.status == ValidationStatus.INVALID_SCHEMA
        )

    def test_validate_vignette_without_age_fails(self, validator: IValidationService):
    ...
    def test_validate_medically_accurate_card(self, validator: IValidationService):
    ...
    def test_validate_medically_inaccurate_card(self, validator: IValidationService):
    ...
    def test_validate_detects_hallucination(self, validator: IValidationService):
        ...
        assert (
            ...
            or result.status == ValidationStatus.HALLUCINATION_DETECTED
        )

    def test_validate_low_hallucination_for_real_content(self, validator: IValidationService):
    ...
    def test_detect_exact_duplicate(self, validator: IValidationService):
        """Exact duplicate cards are detected."""
        card = ClozeCard(
            id="test1", text="The heart has {{c1::four}} chambers", source_chunk_id="chunk_1"
        )

        existing = [
            ClozeCard(
                id="existing", text="The heart has {{c1::four}} chambers", source_chunk_id="chunk_2"
            )
        ]
    ...
    def test_detect_semantic_duplicate(self, validator: IValidationService):
        """Semantically similar cards are detected as duplicates."""
        card = ClozeCard(
            id="test1", text="The heart contains {{c1::4}} chambers", source_chunk_id="chunk_1"
        )
    ...
    def test_not_duplicate_different_content(self, validator: IValidationService):
        ...
        existing = [
            ClozeCard(
                id="existing", text="The lungs have {{c1::five}} lobes", source_chunk_id="chunk_2"
            )
        ]
...
class TestTagBuilding:
    ...
    def test_build_tags_from_topics(self, exporter: IExportService):
    ...
    def test_tags_use_hierarchical_format(self, exporter: IExportService):
    ...
    def test_tags_start_with_hash(self, exporter: IExportService):
    ...
    def test_usmle_tags_anking_compatible(self, exporter: IExportService):
    ...
    def test_mcat_tags_include_foundational_concept(self, exporter: IExportService):
    ...
    def test_source_tag_included(self, exporter: IExportService):
...
class TestDeckBuilding:
    ...
    def test_build_deck_creates_file(
        self, exporter: IExportService, sample_cards: list[ClozeCard], tmp_path: Path
    ):
        """Deck building creates an .apkg file."""
        output = exporter.build_deck(sample_cards, ExamType.USMLE_STEP1, "Test Deck")
    ...
    def test_deck_has_stable_ids(self, exporter: IExportService, sample_cards: list[ClozeCard]):
    ...
    def test_deck_includes_media(
        self, exporter: IExportService, cards_with_images: list[ClozeCard]
    ):
        """Deck includes referenced media files."""
        output = exporter.build_deck(cards_with_images, ExamType.USMLE_STEP1, "Test Deck")
    ...
    def test_deck_name_matches_exam_type(
        self, exporter: IExportService, sample_cards: list[ClozeCard]
    ):
        """Deck name reflects the exam type."""
        output_usmle = exporter.build_deck(sample_cards, ExamType.USMLE_STEP1, "USMLE Step 1")

        output_mcat = exporter.build_deck(sample_cards, ExamType.MCAT, "MCAT Prep")
...
class TestCardGuidGeneration:
    ...
    def test_guid_is_deterministic(self, exporter: IExportService):
        """Same content produces same GUID."""
        card = ClozeCard(
            id="test", text="The heart has {{c1::four}} chambers", source_chunk_id="chunk_1"
        )

        # Build twice
        guid1 = (
            exporter._generate_guid(card.text)
            if hasattr(exporter, "_generate_guid")
            else hashlib.sha256(card.text.encode()).hexdigest()[:20]
        )
        guid2 = (
            exporter._generate_guid(card.text)
            if hasattr(exporter, "_generate_guid")
    ...
    def test_guid_differs_for_different_content(self, exporter: IExportService):
...
class TestEndToEndPipeline:
    ...
    def test_pipeline_handles_multiple_files(
        ...
        result = pipeline.run(
            input_path=pdf_directory, exam_type="mcat", cards_per_chunk=1, output_dir=tmp_path
        )

        assert result.document_count > 1

    def test_pipeline_respects_exam_type(self, pipeline, simple_medical_pdf: Path, tmp_path: Path):
        """Pipeline uses correct taxonomy for exam type."""
        pipeline.run(
            input_path=simple_medical_pdf, exam_type="mcat", cards_per_chunk=1, output_dir=tmp_path
        )
    ...
    def test_pipeline_validates_cards(self, pipeline, simple_medical_pdf: Path, tmp_path: Path):
    ...
    def test_dry_run_does_not_create_file(self, pipeline, simple_medical_pdf: Path, tmp_path: Path):
...
class TestChunkToCardFlow:
    ...
    def test_chunk_classification_to_card_tags(
        ...
        classified = ClassifiedChunk(
            chunk=chunk, usmle_topics=topics, primary_exam=ExamType.USMLE_STEP1
        )
...
class TestErrorRecovery:
    ...
    def test_pipeline_continues_on_chunk_error(
        self, pipeline, pdf_with_corrupted_section: Path, tmp_path: Path
    ):
    ...
    def test_pipeline_handles_empty_pdf(self, pipeline, empty_pdf: Path, tmp_path: Path):
        """Pipeline gracefully handles empty input."""
        result = pipeline.run(
            input_path=empty_pdf, exam_type="usmle-step1", cards_per_chunk=1, output_dir=tmp_path
        )
...
class TestChunkingProperties:
    ...
    def test_chunking_produces_valid_chunks_any_length(
        self, chunker: IChunkingService, text_length: int
    ):
        ...
        document = Document(
            id="test", source_path="/test.pdf", content_type=ContentType.PDF_NOTES, raw_text=text
        )
    ...
    def test_chunking_idempotent(self, chunker: IChunkingService, sample_document: Document):
...
class TestClozeGenerationProperties:
    ...
    def test_generation_respects_count(
        self, generator: IGenerationService, classified_chunk: ClassifiedChunk, count: int
    ):
...
class TestClassificationProperties:
    ...
    def test_classification_confidence_bounds(
        self, classifier: IClassificationService, sample_chunk: Chunk
    ):
...
class TestTagProperties:
    ...
    def test_tags_no_invalid_characters(self, exporter: IExportService):
...
class TestEdgeCases:
    ...
    def test_chunk_empty_document(self, chunker: IChunkingService):
        """Chunking empty document returns empty list."""
        document = Document(
            id="empty", source_path="/empty.pdf", content_type=ContentType.PDF_NOTES, raw_text=""
        )
    ...
    def test_chunk_whitespace_only_document(self, chunker: IChunkingService):
    ...
    def test_classify_empty_chunk(self, classifier: IClassificationService):
        """Classifying empty chunk returns empty results."""
        chunk = Chunk(
            id="empty", document_id="doc", text="", start_char=0, end_char=0, token_count=0
        )
    ...
    def test_generate_from_minimal_chunk(self, generator: IGenerationService):
        """Generation from minimal content handles gracefully."""
        chunk = Chunk(
            id="minimal", document_id="doc", text="Heart.", start_char=0, end_char=6, token_count=1
        )

        classified = ClassifiedChunk(
            chunk=chunk, usmle_topics=[], primary_exam=ExamType.USMLE_STEP1
        )
    ...
    def test_chunk_unicode_content(self, chunker: IChunkingService):
    ...
    def test_chunk_chemical_formulas(self, chunker: IChunkingService):
    ...
    def test_cloze_with_special_characters(self, validator: IValidationService):
    ...
    def test_chunk_very_long_document(self, chunker: IChunkingService):
    ...
    def test_classify_long_chunk(self, classifier: IClassificationService):
    ...
    def test_chunk_malformed_unicode(self, chunker: IChunkingService):
    ...
    def test_validate_malformed_cloze(self, validator: IValidationService):
        """Validation handles malformed cloze syntax."""
        card = ClozeCard(
            id="malformed", text="Missing closing {{c1::bracket", source_chunk_id="chunk_1"
        )
    ...
    @pytest.mark.asyncio
    async def test_concurrent_embedding(self, embedder: IEmbeddingService):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_classification(self, classifier: IClassificationService):
        ...
        tasks = [classifier.classify(chunk, ExamType.USMLE_STEP1) for chunk in chunks]
        results = await asyncio.gather(*tasks)
...
def sample_document() -> Document:
    ...
    return Document(
        ...
        """,
        sections=[Section(title="Cardiovascular Physiology", level=1, start_char=0, end_char=800)],
    )
...
class MockLLMClient:
    ...
    async def generate(self, prompt: str, **kwargs) -> str:
        ...
        if "cloze" in prompt.lower():
            return json.dumps([{"text": "The {{c1::heart}} pumps blood", "extra": "Four chambers"}])
...
class MockVectorStore:
    ...
    async def upsert(
        self, collection: str, vectors: list[list[float]], metadata: list[dict], ids: list[str]
    ) -> None:
        if collection not in self.collections:
            self.collections[collection] = []

        for vec, meta, id_ in zip(vectors, metadata, ids, strict=False):
            self.collections[collection].append({"id": id_, "vector": vec, "metadata": meta})
