...
class CardValidator:
    ...
    def validate_schema(self, card: ClozeCard | VignetteCard) -> ValidationResult:
    ...
    def _validate_cloze_schema(self, card: ClozeCard) -> ValidationResult:
        issues = []
    ...
    def _validate_vignette_schema(self, card: VignetteCard) -> ValidationResult:
        issues = []
    ...
    async def validate_accuracy(self, card: ClozeCard | VignetteCard) -> ValidationResult:
        ...
        text = card.text if isinstance(card, ClozeCard) else card.stem
    ...
    async def validate_grounding(self, card: ClozeCard | VignetteCard) -> ValidationResult:
        if not self.llm_client:
            return ValidationResult(
                status=ValidationStatus.NEEDS_REVIEW,
