...
class TestGenerateCommand:
    ...
    def test_generate_accepts_file(self, tmp_path):
        ...
        with patch("medanki_cli.commands.generate.process_document", new_callable=AsyncMock) as mock_process:
            mock_process.return_value = {"cards": [], "stats": {"total": 0, "cloze": 0, "vignette": 0, "chunks": 0, "duration": 0}}
    ...
    def test_generate_output_option(self, tmp_path):
        ...
        with patch("medanki_cli.commands.generate.process_document", new_callable=AsyncMock) as mock_process:
            mock_process.return_value = {"cards": [], "stats": {"total": 0, "cloze": 0, "vignette": 0, "chunks": 0, "duration": 0}}
            result = runner.invoke(app, ["generate", "--input", str(test_file), "--output", str(output_path), "--dry-run"])
    ...
    def test_generate_exam_option(self, tmp_path):
        ...
        with patch("medanki_cli.commands.generate.process_document", new_callable=AsyncMock) as mock_process:
            mock_process.return_value = {"cards": [], "stats": {"total": 0, "cloze": 0, "vignette": 0, "chunks": 0, "duration": 0}}
            result = runner.invoke(app, ["generate", "--input", str(test_file), "--exam", "USMLE", "--dry-run"])
    ...
    def test_generate_shows_progress(self, tmp_path):
        ...
        with patch("medanki_cli.commands.generate.process_document", new_callable=AsyncMock) as mock_process:
            mock_process.return_value = {"cards": [], "stats": {"total": 0, "cloze": 0, "vignette": 0, "chunks": 0, "duration": 0}}
            result = runner.invoke(app, ["generate", "--input", str(test_file), "--verbose", "--dry-run"])

        assert result.exit_code == 0
        assert "processing" in result.stdout.lower() or "complete" in result.stdout.lower() or "results" in result.stdout.lower()
    ...
    def test_generate_creates_apkg(self, tmp_path):
        ...
        with patch("medanki_cli.commands.generate.process_document", new_callable=AsyncMock) as mock_process:
            with patch("medanki_cli.commands.generate.create_apkg") as mock_apkg:
                mock_process.return_value = {"cards": [{"text": "test"}], "stats": {"total": 1, "cloze": 1, "vignette": 0, "chunks": 1, "duration": 0}}
